...
const nextTurnAfterLeave = function(roomInfo, userId){
    if (roomInfo.isPlaying > -1){
        const turn = roomInfo.isPlaying;
        const idList = Array.from(roomInfo.playersCard.keys());
        var order = idList.indexOf(userId);
        if (order < 0){
            return false;
        }
        
        if (order < turn){
            roomInfo.isPlaying -= 2;
            // const turnId = idList[roomInfo.isPlaying];
            // return turnId;
        }
        else {
            roomInfo.isPlaying -= 1;
            // const turnId = idList[roomInfo.isPlaying + 1];
            // return turnId;
        }
        return true;
    ...
    io.on('connect', (socket) => {
        ...
        socket.on('join-room', (roomId, userId) => {
            ...
            if (gameStore.has(roomIdNumber)) {
                ...
                if (roomInfo.players.has(userIdNumber) && roomInfo.isPlaying < 0) {
                    ...
                    socketMap.set(socket.id, { roomId: roomIdNumber, userId: userIdNumber })
            ...
            if (gameStore.has(roomIdNumber)) {
                ...
                if (roomInfo.dealer.id == userIdNumber) {
                    ...
                    if (roomInfo.players.has(userIdNumber)) {
                        ...
                        socket.to(roomIdNumber).emit('player-leave-room', userIdNumber);
                        const checkTurn = nextTurnAfterLeave(roomInfo, userIdNumber);
                            if (checkTurn) {
                                roomInfo.playersCard.delete(userIdNumber);
                                const turnId = nextTurnId(roomInfo);
        ...
        socket.on('disconnect', function() {
            if (socketMap.has(socket.id)) {
                ...
                const roomIdNumber = +socketMap.get(socket.id).roomId;
                const userIdNumber = +socketMap.get(socket.id).userId;
                if (gameStore.has(roomIdNumber)) {
                    const roomInfo = gameStore.get(roomIdNumber)
                    if (roomInfo.dealer.id == userIdNumber) {
                        socket.to(roomIdNumber).emit('dealer-leave-room');
